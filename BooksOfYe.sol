...
contract BooksOfYe is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    constructor() ERC1155(baseURI){
     ...
     //Public Functions
        ...
        function preSaleMint(uint256 eventNumber, uint cardId) public payable {                      
            ...
            require (saleEvents[eventNumber].isPreSale && !saleEvents[eventNumber].isPublicSale, "PreSale Is Not Live"); 
            require (saleEvents[eventNumber].whitelist[msg.sender], "Sorry You Aren't On The Whitelist");
            require (saleEvents[eventNumber].cardsForSale[cardId], "This Card Is Not For Sale");
            require (cardId < 1000, "The Card Number Selected Does Not Exist");
            require(maxSupply - 1 >= 0, "Sorry, There Aren't Anymore Available Cards");
            require(!exists(cardId), "Sorry This Card Is Already Minted By Someone Else");
            require(cardPurchaseTracker[msg.sender] != mintLimit, "Sorry, You've Hit The Mint Limit");
            ...
            if (nextEventNumber <= 4){
                ...
                saleEvents[nextEventNumber].whitelistCount = saleEvents[nextEventNumber].whitelistCount + 1;
            }   
        }
    
        function publicMint(uint256 eventNumber, uint cardId) public payable {                     
            uint256 nextEventNumber = eventNumber + 1;
            uint256 price = saleEvents[eventNumber].price;
            
            require (saleEvents[eventNumber].isActive, "Sale Is Not Active");
            require (!saleEvents[eventNumber].isPreSale && saleEvents[eventNumber].isPublicSale, "Public Sale Is Not Live"); 
            require (saleEvents[eventNumber].cardsForSale[cardId], "This Card Is Not For Sale");
            require (cardId < 1000, "The Card Number Selected Does Not Exist");
            require(maxSupply - 1 >= 0, "Sorry, There Aren't Anymore Available Cards");
            require(!exists(cardId), "Sorry This Card Is Already Minted By Someone Else");
        ...
        function activateCardPerks(uint256 cardId, uint256 eventNumber) public {
            ...
            require(balanceOf(msg.sender, cardId ) > 0, "You Don't Have This Card In Your Wallet.");
            require(!saleEvents[eventNumber].isActive , "Please Wait For The Current Sale To Be Over.");
        ...
        function checkWhitelist (uint256 eventNumber, address _address) public view returns(bool isUserWL) {
           isUserWL = saleEvents[eventNumber].whitelist[_address];
           return isUserWL;

        }

        function checkTokenOwners(uint256 cardId) public view returns (address ownerOfToken ){
     ...
     //Only Owner Functions
        ...
        function giftMint(address _address, uint cardId, uint addToWhichWhitelist) public onlyOwner {            
                
                require (cardId < 1000, "The Card Number Selected Does Not Exist");
                require(maxSupply - 1 >= 0, "Sorry, There Aren't Anymore Available Cards");
                require(!exists(cardId), "Sorry This Card Already Belongs To Someone Else");
                    
                _mint(_address, cardId, 1, "");
                tokenOwners[cardId] = _address;
                maxSupply = maxSupply - 1;
                saleEvents[addToWhichWhitelist].whitelist[_address] = true;
                saleEvents[addToWhichWhitelist].whitelistCount = saleEvents[addToWhichWhitelist].whitelistCount + 1;  
            }

        function batchGiftMint(address[] memory _addresses, uint[] memory cardId, uint addToWhichWhitelist) public onlyOwner {            
            
            for(uint256 i = 0; i < _addresses.length; i++){
                require (cardId[i] < 1000, "The Card Number Selected Does Not Exist");
                require(maxSupply - cardId.length >= 0, "Sorry, There Aren't Anymore Available Cards");
                require(!exists(cardId[i]), "Sorry This Card Already Belongs To Someone Else");
        ...
        function editSaleStatus(uint256 eventNumber, bool _isActive, bool _isPreSale, bool _isPublicSale) 
            public onlyOwner  {
        ...
        function viewSaleStatus(uint256 eventNumber) public onlyOwner view returns
        (uint256 price, uint256 whitelistCount, bool isActive, bool isPreSale, bool isPublicSale){
        ...
        function editSalePrice(uint256 eventNumber, uint256 _newPriceInWei) public onlyOwner{
            
            require(eventNumber <= 4);
            saleEvents[eventNumber].price = _newPriceInWei;
        }

        function addCardToSale(uint256 eventNumber, uint256 cardId) public onlyOwner{
            require(eventNumber <= 4);
            require(!saleEvents[eventNumber].cardsForSale[cardId]);
            saleEvents[eventNumber].cardsForSale[cardId] = true;
        }

        function removeCardFromSale(uint256 eventNumber, uint256 cardId) public onlyOwner{
            require(eventNumber <= 4);    
            require(saleEvents[eventNumber].cardsForSale[cardId]);
            saleEvents[eventNumber].cardsForSale[cardId] = false;
        }

        function batchAddToWhitelist (address[] memory _addresses, uint256 eventNumber) public onlyOwner{
            for(uint256 i = 0; i < _addresses.length; i++){
        ...
        function batchRemoveFromWhitelist (address[] memory _addresses, uint256 eventNumber) public onlyOwner{
            for(uint256 i = 0; i < _addresses.length; i++){
        ...
        function addToWhitelist(address _address, uint256 eventNumber) public onlyOwner{
            saleEvents[eventNumber].whitelist[_address] = true;
            saleEvents[eventNumber].whitelistCount = saleEvents[eventNumber].whitelistCount + 1;

        }

        function removeFromWhitelist(address _address, uint256 eventNumber) public onlyOwner{
